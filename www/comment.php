...
echo '<div id="newswrap">';
    ...
    echo '<div class="comments">';
        ...
        Haanga::Load('share.html', array('link' => $globals['permalink'], 'title' => $title));
        echo '</div>';
...
function fill_tree($tree, $limit = 30)
{
    ...
    $res = $db->get_results(DbHelper::queryPlain('
        SELECT conversation_to AS parent, conversation_from AS child
        FROM conversations
        WHERE (
            conversation_type = "comment"
            AND conversation_to IN ('.DbHelper::implodedIds($parents).')
        )
        LIMIT '.(int)$limit.';
    '));
