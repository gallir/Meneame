...
$type = $_REQUEST['type'];
if ($type != 'link' && $type != 'comment' && $type != 'user' && $type != 'author' && $type != 'post') {
    $type = 'link';
}

$status = $_REQUEST['status'];
if (!empty($status) && $status != 'published' && $status != 'all' && $status != 'queued') {
    $status = false;
}
...
switch ($type) {
    case 'link':
        if ($id > 0) {
            $cond = add_cond($cond, "link_id = $id");
        } else {
            if ($from > 0) {
                $cond = add_cond($cond, "link_date > date_sub(now(), interval $from hour)");
            }
            if ($status) {
                $cond = add_cond($cond, "link_status = '$status'");
            }
        }
    ...
    case 'post':
        if ($id > 0) {
            $cond = add_cond($cond, "post_id = $id");
        }
        if ($from > 0) {
            $cond = add_cond($cond, "post_date > date_sub(now(), interval $from hour)");
        }
