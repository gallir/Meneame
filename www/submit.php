...
function do_submit1() {
    ...
    if (empty($_POST['url'])) {
        ...
        if (strlen($url) > 250) {
            add_submit_error( _('URL demasiado larga'), _('La longitud de la URL supera el tamaño máximo permitido (250 caracteres)'));
            return false;
        }
    }
    ...
    if ($drafts > $globals['draft_limit']) {
        add_submit_error( _('demasiados borradores'), _('has hecho demasiados intentos, debes esperar o continuar con ellos desde la'). ' <a href="queue?meta=_discarded">'. _('cola de descartadas').'</a></p>');
    ...
    if ($new_user && $globals['min_user_votes'] > 0 && $current_user->user_karma < $globals['new_user_karma']) {
        ...
        if (!$current_user->admin && $user_votes < $min_votes) {
            ...
            add_submit_error( _('no votes de forma apresurada, penaliza el karma'), '<a href="'.$globals['base_url'].'queue" target="_blank">'._('haz clic aquí para ir a votar').'</a>');
    ...
    if (!empty($link->url) ) {
        if (report_duplicated($url)) {
            return true; // Don't output error messages
        }
    ...
    if ($anti_spam) {
        ...
        if ($same_blog > 0 && $current_user->user_karma < 12) {
            ...
            add_submit_error( _('ya has enviado un enlace al mismo sitio hace poco tiempo'), _('debes esperar'). " $minutes " . _('minutos entre cada envío al mismo sitio.') . ', ' . '<a href="'.$globals['base_url_general'].'faq-'.$dblang.'.php">'._('lee el FAQ').'</a>');
            return false;
        }
        ...
        if (! $site_info->owner) { // Only for the main subs
            ...
            if ($site_links > 10 && $site_links > $links_12hs * 0.05) { // Only 5% from the same site
                ...
                add_submit_error( _('hay en cola demasiados envíos del mismo sitio, espera unos minutos por favor'), _('total en 12 horas').": $site_links , ". _('el máximo actual es'). ': ' . intval($links_12hs * 0.05));
                return false;
            }
            ...
            if (($ban = check_ban($link->url, 'punished_hostname', false, true))) {
                add_submit_error( _('Aviso').' '.$ban['match']. ': <em>'.$ban['comment'].'</em>', _('mejor enviar el enlace a la fuente original'));
            }
        }
    } // END anti_spam
...
function do_submit3() {
    ...
    if (empty($site_properties['no_link']) && Link::duplicates($link->url)) {
