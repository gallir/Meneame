...
class BasicThumb
{
    ...
    public function __construct($url='', $referer=false)
    {
        ...
        if ($referer) {
            $this->parsed_referer = parse_url($referer);
        }
...
// New code to get rectangular images
    ...
    public function fromstring(&$imgstr)
    {
        $this->checked = true;
...
class WebThumb extends BasicThumb
{
    ...
    public function __construct($imgtag = '', $referer = '')
    {
        if (!$imgtag) {
            return;
        }
        ...
        if (!preg_match('/(?:src|poster) *=["\'](.+?)["\']/i', $this->tag, $matches)
            ...
            if (preg_match('/usemap=|header/i', $this->tag)) {
                return;
            }
        }
        ...
        if (strlen($this->url) < 5 || WebThumb::$visited[$this->url]) {
            return;
        }
...
class HtmlImages
{
    ...
    public function __construct($url, $site = false)
    {
        global $db;
        $this->url = $url;
    ...
    public function parse_img(&$html)
    {
        ...
        foreach ($tags as $match) {
            ...
            if ($this->seen($img->url) || ($img->surface() < 120000 && $this->check_in_other($match))) {
                continue;
            }
            if ($this->debug) {
    ...
    public function get_other_html()
    {
        $selection = array();
        $levels = array();
        $level_max = array();
    ...
    public function check_in_other($str, $times = 1)
    {
        $n = 0;
    ...
    public function get_google_thumb($videoid)
    {
    ...
    public function get_youtube_thumb($videoid)
    {
        ...
        if (preg_match('/yfrog\.com/', $this->parsed_url['host'])) {
            ...
            if ($url) {
                ...
                if ($img->get()) {
                    ...
                    $this->selected = $img;
                    if ($this->debug) {
                        echo "<!-- Thumb selected from $img->url -->\n";
                    }
                    return $this->selected;
                }
            }
    ...
    public function get_metacafe_thumb($videoid)
    {
    ...
    public function get_vimeo_thumb($videoid)
    {
    ...
    public function get_zapp_internet_thumb($videoid)
    {
    ...
    public function get_daily_motion_thumb($videoid)
    {
