...
function geo_distance($from, $to)
{
    ...
    return acos(sin($latFrom) * sin($latTo) + cos($latFrom) * cos($latTo) * cos($lngTo - $lngFrom)) * $er;
}
...
function geo_delete($type, $id)
{
    ...
    return $db->query('DELETE FROM '.$table.' WHERE geo_id = "'.(int)$id.'";');
}
...
function geo_init($f = 'geo_basic_load', $latlng = false, $zoom = 7, $icontype = 'user')
{
    ...
    array_push($globals['extra_js'], '//maps.google.com/maps?file=api&amp;v=2&amp;key=' . $globals['google_maps_api']);
    array_push($globals['extra_js'], 'geo.js');

    if ($f) {
        if ($latlng) {
            add_javascript($f . '(' . $latlng->lat . ',' . $latlng->lng . ', ' . $zoom . ', \'' . $icontype . '\');');
            $globals['body_args'] = 'onunload="GUnload()"';
        } else {
            add_javascript($f . '(false, false, ' . $zoom . ', \'' . $icontype . '\');');
            $globals['body_args'] = 'onunload="GUnload()"';
        }
    } else {
        $globals['body_args'] = 'onunload="GUnload()"';
