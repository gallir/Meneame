...
class Blog
{
    ...
    public static function find_blog($url, $to_check = 0)
    {
        global $db;
    ...
    public function analyze_html($url, $html)
    {
        ...
        if ($this->rss || $this->atom || $this->rss2) {
            $this->type='blog';
        }
    ...
    public function getShortestValFromExp($match, $secondMatch, $text)
    {
        $value=false;
    ...
    public function find_base_url($url)
    {
        ...
        if (empty($url_url['scheme'])) {
            $scheme="http";
        } else {
            $scheme=$url_url['scheme'];
        }
        $this->url=$scheme.'://'.$host.$path;
    }

    public function shortest_text($strArray)
    {
        $shortest=false;
