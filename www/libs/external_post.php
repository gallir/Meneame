...
function twitter_post($auth, $text, $short_url, $image = false)
{
    ...
    if (empty($auth['twitter_token']) || empty($auth['twitter_token_secret']) || empty($auth['twitter_consumer_key']) ||  empty($auth['twitter_consumer_secret']) || $globals['twitter_autopublish'] === false) {
        return false;
    }
...
function facebook_post($auth, $link, $text = '')
{
    ...
    if (empty($auth['facebook_token']) || empty($auth['facebook_key']) || empty($auth['facebook_secret']) ||  $globals['facebook_autopublish'] === false) {
        return false;
    }
