...
class SimpleImage
{
    ...
    public function save($filename, $image_type=-1, $compression=90)
    {
        if (!$this->image) {
    ...
    public function resize($width, $height, $crop = false)
    {
        ...
        if (! @imagecopyresampled($new_image, $this->image, 0, 0, $src_x, $src_y, $width, $height, $src_w, $src_h)) {
            return false;
        }
    ...
    public function rotate_exif($filename = false)
    {
        ...
        if (! $this->image || $this->image_type != IMAGETYPE_JPEG) {
            return false;
        }
