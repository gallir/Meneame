...
if ($logs) {
    foreach ($logs as $log) {
        ...
        switch ($log->log_type) {
            case 'link_new':
                if (empty($_REQUEST['nonew'])) {
                    get_story($log->time, 'new', $log->log_ref_id, $log->log_user_id);
                }
                break;
            case 'link_publish':
                if (empty($_REQUEST['nopublished'])) {
                    get_story($log->time, 'published', $log->log_ref_id, $log->log_user_id);
                }
                break;
            case 'comment_new':
                if (User::friend_exists($current_user->user_id, $log->log_user_id) < 0) {
            ...
            case 'link_discard':
                if (empty($_REQUEST['noproblem']) && empty($_REQUEST['nodiscard'])) {
                    get_story($log->time, 'discarded', $log->log_ref_id, $log->log_user_id);
                }
                break;
            case 'link_edit':
                if (empty($_REQUEST['noedit']) && ($current_user->admin || $current_user->user_level == 'blogger')) {
                    get_story($log->time, 'edited', $log->log_ref_id, $log->log_user_id);
                }
                break;
            case 'link_geo_edit':
                if (empty($_REQUEST['nogeoedit']) && $current_user->admin) {
            ...
            case 'comment_edit':
                ...
                if (empty($_REQUEST['nocomment'])) {
                    get_comment($log->time, 'cedited', $log->log_ref_id, $log->log_user_id);
                }
                break;
            case 'post_new':
                if (User::friend_exists($current_user->user_id, $log->log_user_id) < 0) {
...
function get_chat()
{
    ...
    if (!empty($_REQUEST['admin']) || !empty($_REQUEST['friends'])) {
        $chat_items = $max_items * 2;
    } else {
        $chat_items = $max_items;
    }
    ...
    foreach ($res as $event) {
        ...
        if ($uid != $current_user->user_id) {
            ...
            if ($event->chat_room == 'admin' && ! $current_user->admin) {
                continue;
            }
        ...
        if ($uid >0) {
            $json['icon'] = get_avatar_url($uid, -1, 20);
        }
        $key = $event->chat_time . ':chat:'.$uid;

        $events[$key] = $json;
        if ($event->chat_time > $last_timestamp) {
            $last_timestamp = $event->chat_time;
        }
    }
}
