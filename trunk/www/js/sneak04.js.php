...
function sneak_tooltip_get_text(event, script, id) {
	var initial_mess = "<?echo _('cargando...');?>";
	tooltip.c_show(event, 'text', initial_mess);
	tooltip_timeout = setTimeout("sneak_tooltip_ajax_request('"+script+"', "+id+")", 300);
}

function sneak_tooltip_clear(event) {
	if (tooltip_timeout != null) {
...
function sneak_tooltip_ajax_request(script, id) {
	tooltip_timeout = null;
