...
echo '<html><head><title>promote4.php</title></head><body style="font-size: 10pt;" >';
...
/////////////
	...
	$limit_karma = round($past_karma * 0.6);
	...
	$best_link = 0;
	$best_karma = 0;
	echo "<table cellpadding=2 style=\"font-size: 10pt;\" >\n";	
	if ($links) {
		...
		foreach($links as $dblink) {
			...
			if ($link->votes >= $min_votes &&
				$dblink->karma > $max_karma_found  ) {
				$max_karma_found = $dblink->karma;
		...
		if ($best_link > 0 && $best_karma >= $min_karma) {
			$i++;
			$link->id = $best_link;
			$link->read();
