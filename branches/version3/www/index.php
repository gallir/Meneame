...
function print_index_tabs($option=-1) {
	...
	echo '<li '.$active[0].' href="'.$globals['base_url'].$globals['meta_skip'].'">'._('todas'). '</a></li>'."\n";
	...
	if ($metas) {
		foreach ($metas as $meta) {
			...
			echo '<li '.$active_meta.' href="'.$globals['base_url'].'?meta='.$meta->category_uri.'">'.$meta->category_name. '</a></li>'."\n";
		}
	}
	if ($current_user->user_id > 0) {
		echo '<li '.$active[1].' href="'.$globals['base_url'].'?meta=_friends">'._('amigos'). '</a></li>'."\n";
	} else {
		meta_teaser_item();
	}
	...
	switch ($option) {
		...
		case 7: // Personalised, published
			echo '<li><a class="teaser" href="'.$globals['base_url'].'rss2.php?personal='.$current_user->user_id.'" rel="rss"><img src="'.$globals['base_url'].'img/common/feed-icon-12x12.png" width="12" height="12" alt="rss2"/></a></li>';
			break;
		default:
