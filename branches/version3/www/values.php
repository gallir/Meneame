...
echo '
		<fieldset>
			<legend>karma</legend>
				...
				'._("El karma de un usuario puede ir de ") . $globals['min_karma'] . _(" a ") . $globals['max_karma'] . '.<br />
				<br />
...
if($globals['min_karma_for_links']) {
				echo _("Karma mínimo para enviar historias (") ._('no tienes el mínimo de karma para enviar una nueva historia'). "): " . $globals['min_karma_for_links'] . '.<br />
				<br />
				';
}

if($globals['min_karma_for_comments']) {
				echo _("Karma mínimo para enviar comentarios ("). _('No tienes el mínimo karma requerido') . "): ". $globals['min_karma_for_comments'] . '.<br />
				<br />
				';
}

if($globals['min_karma_for_comment_votes']) {
				echo _("Karma mínimo para votar comentarios ("). _('No tienes el mínimo karma requerido') ."): " . $globals['min_karma_for_comment_votes'] . '.<br />
				<br />
				';
}

if($globals['min_karma_for_posts']) {
				echo _("Karma mínimo para enviar nótames (") . _('el karma es muy bajo') . "): " . $globals['min_karma_for_posts'] . '.<br />
				<br />
				';
}

if($globals['min_karma_for_sneaker']) {
				echo _("Karma mínimo para hablar en la fisgona (") . _('no tienes suficiente karma para comentar en la fisgona') . "): " . $globals['min_karma_for_sneaker'] . '.<br />
				<br />
				';
}

echo				_("Karma instantáneo (sin esperar al recalculo diario) ganado por un usuario en el momento que se publica uno de sus envíos: ") . $globals['instant_karma_per_published'] . '.<br />
				<br />
...
echo '
		<fieldset>
			<legend>votos abiertos</legend>
				'._("Tiempo que permanecen abiertos los votos: ") . print_time($globals['time_enabled_votes']) . '.<br />
				<br />
				'._("Tiempo abiertos los negativos si se ha extendido el tiempo de voto negativo (lleva aviso o negativos > positivos/10): ") . print_time($globals['time_enabled_votes']) . '.<br />
				<br />
...
echo '
		<fieldset>
			<legend>envíos</legend>
				...
				<br />
				'._("El karma de una noticia que sufre un \"depublish\" y vuelve a pendientes pasa a ser un "). intval(100 / $globals['depublish_karma_divisor']) . _("% del karma anterior") . '.<br />
		</fieldset>';
