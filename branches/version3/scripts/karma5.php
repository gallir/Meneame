...
$max_published_given = $db->get_var("SELECT SQL_NO_CACHE count(*) as votes from links, votes  where vote_type='links' and  vote_date > $history_from and vote_user_id > 0 and vote_value>0 and vote_link_id = link_id and link_status='published' and vote_date < date_sub(link_published_date, INTERVAL 60 minute) group by vote_user_id order by votes desc limit 1");
...
foreach($users as $dbuser) {
	...
	if ($n > 3 || $n_events > 0) {
		...
		print "$user->username (votes-karma received) ($positive_votes_received, $negative_votes_received): $karma1\n";

/////

		$published_given = $db->get_var("SELECT SQL_NO_CACHE count(*) FROM votes,links WHERE vote_type='links' and vote_user_id = $user->id and vote_date > $history_from  and vote_value > 0 AND link_id = vote_link_id AND link_status = 'published' AND vote_date < date_sub(link_published_date, interval 90 minute)");
		...
		// Bot warning!!!
		if ($published_links > 5 && $published_given > $published_links/3 && 
			($published_given > $nopublished_given * 5 || ($published_given > $nopublished_given * 2 && $total_comments == 0 && $sent_links == 0) )) {
			if ($published_given > $nopublished_given * 5) {
				$karma2 = -3; // Be stronger against  users whithout comments or links, bots bots bots
				print "$user->username BOT suspected, karma2 = $karma2\n";
			} else {
				$karma2 = 0;  // At least left a a comment, possibly a smart karma whore or manually supported bot
				print "$user->username karma whore suspected, karma2 = $karma2\n";
			}
		}  else {
