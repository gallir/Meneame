...
$sql = "SELECT link_id, link_url, link_votes, link_negatives, link_karma from links WHERE link_date > DATE_SUB(now(), INTERVAL $days DAY) AND link_status != 'discard' ORDER BY link_karma DESC, link_votes DESC LIMIT $rows";
$link = new Link;
$links = $db->get_results($sql);
if ($links) {
	header('Content-Type: text/plain');
	foreach($links as $link) {
		echo "$link->link_url\t$link->link_votes\t$link->link_negatives\t$link->link_karma\n";
	}
}
?>
