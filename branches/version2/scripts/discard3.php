...
// send back to queue erroneus or plagiarism that were published
$links = $db->get_results("select link_id, link_author, link_date, link_karma from links where link_status = 'published' and link_date > date_sub(now(), interval 1 hour) and link_negatives > link_votes / 4");

if ($links) {
	foreach ($links as $link) {
		// count those votes with $globals['negative_votes_values'] = -8 =>  _('errónea'),  -9 => _('copia/plagio')
		$negatives = (int) $db->get_var("select sum(user_karma) from votes, users where vote_type='links' and vote_link_id=$link->link_id and vote_date > '$link->link_date' and vote_value in (-8, -9) and vote_user_id > 0 and user_id = vote_user_id");
