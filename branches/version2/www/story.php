...
case 2:
	...
	if($link->date < $globals['now']-$globals['time_enabled_comments']) {
		...
		if ($current_user->authenticated && $current_user->user_karma <= $globals['min_karma_for_comments']) 
			...
			echo '<a href="'.$globals['base_url'].'login.php?return='.$_SERVER['REQUEST_URI'].'">'._('Autentifícate si deseas escribir').'</a> '._('comentarios').'. '._('O regístrate'). ' <a href="'.$globals['base_url'].'register.php">aquí</a><p><em>¿Qué beneficios tienes registrando una cuenta en menéame?<em><ul><li>Enviar las noticias interesantes que encuentres en la Red.</li><li>Expresar tu opinión escribiendo comentarios.</li><li>Reflejar en tu perfil las noticias que te interesan, votándolas o enviándolas.</li><li>Recordar las noticias que más te interesan.</li><li>Formar parte de una red de usuarios de menéame con gustos afines.</li></ul></p>'."\n";
		echo '</div>'."\n";
	}
	echo '</div>' . "\n";
	break;
