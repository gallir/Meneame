...
function save_post ($post_id) {
	...
	if ($post_id > 0) {
		...
		if (!$already_stored) {
			...
			if(intval($db->get_var("select count(*) from posts where post_user_id = $current_user->user_id and post_date > date_sub(now(), interval 5 minute)"))> 0) {
				echo 'ERROR: ' . _('debe esperar 5 minutos entre apuntes');
				die;
			};
