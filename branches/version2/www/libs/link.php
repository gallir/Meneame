...
class Link {
	...
	function print_summary($type='full') {
		...
		if($type=='full' || $type=='preview') {
			...
			if ($type != 'preview' ) {
				if ($this->is_editable()) {
					echo '&nbsp;&nbsp;<a href="'.$globals['base_url'].'editlink.php?id='.$this->id.'&amp;user='.$current_user->user_id.'" title="'._('editar noticia').'"><img src="'.$globals['base_url'].'img/common/edit-misc01.png" alt="edit"/></a>';
				}
