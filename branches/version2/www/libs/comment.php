	...
	function print_summary($link, $length = 0, $single_link=true) {
		...
		if ($this->type != 'admin' && $this->user_level != 'disabled') {
			...
			if ($this->votes > 0 && $this->date > $globals['now'] - 30*86400) { // Show votes if newer than 30 days
				...
				echo '<img src="'.$globals['base_url'].'img/common/vote-info01.png" width="12" height="12" alt="+ info"/>';
				echo '</a>';
			}
			echo '</div>';
		}
...
//TODO
	...
	function put_comment_tooltips($str) {
		return preg_replace('/(^|[\(\s])#([1-9][0-9]*)/', "$1<a class='tt' href=\"".$this->link_permalink."/000$2\" onmouseover=\"return tooltip.c_show(event, 'id', '$2', '".$this->link."');\" onmouseout=\"tooltip.hide(event);\"  onclick=\"tooltip.hide(this);\">#$2</a>", $str);
	}
