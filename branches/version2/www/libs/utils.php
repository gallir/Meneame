...
function text_to_html($string) {
	...
	$string = preg_replace('/(^|[\W\s])_([^\s]+)_([\W\s]|$)/', "$1<em>$2</em>$3", $string);
	$string = preg_replace('/(^|[\W\s])\*([^\s]+)\*([\W\s]|$)/', "$1<strong>$2</strong>$3", $string);
	return $string;
}
...
function print_simpleformat_buttons($textarea_id) {
	global $globals;
	echo '<img onclick="applyTag(\''.$textarea_id.'\', \'*\');" src="'.$globals['base_url'].'img/common/richeditor-bold-01.png" alt="bold" />';
    echo ' <img onclick="applyTag(\''.$textarea_id.'\', \'_\');" src="'.$globals['base_url'].'img/common/richeditor-italic-01.png" alt="italic" />';
}
?>
