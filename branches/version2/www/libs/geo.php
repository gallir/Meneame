...
function geo_print_form($type, $id, $latlng, $label) {
	...
	echo '<form action="#" name="geoform" onsubmit="geo_show_address(this.address.value); return false">';
	echo '<label for="address">'.$label. '</label><br/>';
	echo '<input type="text" size="50" name="address" id="address" value="'.$latlng->text.'" />';
	echo '&nbsp;<input type="button" value="'._('buscar').'" class="genericsubmit" onclick="geo_show_address(geoform.address.value); return false"/>';
	echo '&nbsp;<input type="button" value="'._('grabar').'" class="genericsubmit" onclick="geo_save_current(\''.$type.'\', '.$id.'); return false"/>';
	echo '&nbsp;<input type="button" value="'._('borrar').'" class="genericsubmit" onclick="geo_delete(\''.$type.'\', '.$id.'); return false"/>';
